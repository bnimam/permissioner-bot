...
class CBOT:
    def __init__(self, net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn) -> None:
        self.net = net
        self.sess = sess
        self.chars = chars
        self.vocab = vocab
        ...
        self.topn = topn
        self.states = self.initial_state_with_relevance_masking(
        ...
        if prime_text is not None:
            for char in prime_text:
                if relevance > 0.:
                    ...
                    if vocab[char] == '\n':
                        states[1] = CBOT.initial_state(net, sess)
                    else:
    ...
    def initial_state_with_relevance_masking(net, sess, relevance):
        if relevance <= 0.:
            return CBOT.initial_state(net, sess)
        else:
            return [CBOT.initial_state(net, sess), CBOT.initial_state(net, sess)]
    ...
    def proc_disc_message(self, text_input=''):
        user_input = text_input
        user_command_entered, reset, self.states, self.relevance, self.temperature, self.topn, self.beam_width = self.process_user_command(
            user_input, self.states, self.relevance, self.temperature, self.topn, self.beam_width)
        if reset:
            self.states = self.initial_state_with_relevance_masking(
                self.net, self.sess, self.relevance)
        if not user_command_entered:
            self.states = self.forward_text(self.net, self.sess, self.states, self.relevance, self.vocab, self.sanitize_text(
                self.vocab, "> " + user_input + "\n>"))
            ...
            for i, char_token in enumerate(computer_response_generator):
                ...
                out_text += self.possibly_escaped_char(out_chars)
                self.states = self.forward_text(self.net, self.sess, self.states,
                                                self.relevance, self.vocab, self.chars[char_token])
                if i >= self.max_length:
                    break
            self.states = self.forward_text(self.net, self.sess, self.states,
                                            self.relevance, self.vocab, self.sanitize_text(self.vocab, "\n> "))
            return out_text
    ...
    def process_user_command(user_input, states, relevance, temperature, topn, beam_width):
        ...
        try:
            if user_input.startswith('--temperature '):
                ...
                temperature = max(0.001, float(
                    user_input[len('--temperature '):]))
    ...
    def beam_search_generator(sess, net, initial_state, initial_sample,
        ...
        while True:
            ...
            l, early_term = CBOT.consensus_length(beam_outputs, early_term_token)
            if l > 0:
    ...
    def forward_with_mask(sess, net, states, input_sample, forward_args):
        ...
        if relevance <= 0.:
            ...
            mask_prob, states[1] = net.forward_model(
                sess, states[1], input_sample)
        ...
        prob = prob / sum(prob)
        # Apply temperature.
        prob = CBOT.scale_prediction(prob, temperature)
